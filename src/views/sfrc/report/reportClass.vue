        ...
        <Grid ref="gridRef" :gap="[20, 0]" :cols="3">
          ...
          <GridItem>
            <el-form-item label="接入方式：" prop="accessMode">
              <el-select
                v-model="formDataAdd.accessMode"
                placeholder="请选择接入方式"
                :disabled="saveType === 'watch'"
                clearable
          ...
          <GridItem v-if="formDataAdd.accessMode === 'API'">
            <el-form-item label="接口版本：" prop="apiVersion">
              <el-select
                v-model="formDataAdd.apiVersion"
                placeholder="请选择接口版本"
                :disabled="saveType === 'watch'"
                style="width: 100%"
              >
          ...
          <GridItem v-if="formDataAdd.accessMode === 'DATABASE'">
            <el-form-item label="接入版本：" prop="accessExchangePlanFlag">
              <el-select
                v-model="formDataAdd.accessExchangePlanFlag"
                placeholder="请选择接入版本"
                :disabled="saveType === 'watch'"
              >
                <el-option
                  v-for="item in accessExchangePlanFlagList"
                  :label="`[${item.planFlag}]${item.planName}`"
                  :value="item.planFlag"
                  :key="item.id"
                ></el-option>
              </el-select>
          ...
          <GridItem>
            <el-form-item label="分类：" prop="reportCategoryCode">
              <el-select
                v-model="formDataAdd.reportCategoryCode"
                placeholder="请选择分类"
                :disabled="saveType === 'watch'"
                @change="reportCategoryCodeChange"
              >
                <el-option
...
<script setup lang="ts" name="reportClass">
