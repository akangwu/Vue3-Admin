<template>
  <div class="table-box">
    ...
    <el-dialog
      v-model="visibleAdd"
      :title="saveType === 'add' ? '新增' : saveType === 'edit' ? '编辑' : '查看明细'"
      width="40%"
      class="dialog-form"
      v-if="visibleAdd"
    >
        ...
        <Grid ref="gridRef" :gap="[20, 0]" :cols="1">
          <GridItem>
            <el-form-item label="分类名称：" prop="categoryName">
              <el-input
                v-model="formDataAdd.categoryName"
                clearable
                placeholder="请输入分类名称"
          ...
          <GridItem>
            <el-form-item label="分类编号：" prop="categoryCode">
              <el-input
                v-model="formDataAdd.categoryCode"
                clearable
                placeholder="请输入分类编号"
          ...
          <GridItem>
            <el-form-item label="报表编码：" prop="reportCode">
              <el-input
                v-model="formDataAdd.reportCode"
                clearable
                placeholder="请输入报表编码"
                :disabled="saveType === 'watch'"
              ></el-input>
...
<script setup lang="ts" name="reportClassConfig">
...
const saveData = async () => {
  formAddRef.value.validate(async valid => {
    ...
    const { code, msg } = await proxy.axios.post(
      saveType.value === "add" ? "/sfrc/api/reportCategory/save" : "/sfrc/api/reportCategory/update",
      params
    );
