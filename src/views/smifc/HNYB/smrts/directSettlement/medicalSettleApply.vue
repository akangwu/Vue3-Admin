<template>
	<div class="table-box">
		...
		<v-table ref="proTable" title="药采结算申请" if-index :column="columns" :data="tableData">
...
const columns = [
	...
	{ label: '审核状态', prop: 'operateState', width: 150 },
	{ label: '审核时间', prop: 'operateTime', width: 150, align: 'center' },
...
const getInsuranceList = async () => {
	...
	formItems.value[3].list = data.map(item => ({
		label: item.chrName,
		value: item.chrCode
	}))
}
...
const getData = async () => {
	...
	const response = await proxy.axios.post(url, params)
	if (response.code === 0) {
		...
		proxy.msg({ type: 'error', message: response.msg })
	}
}
