<template>
	<div class="table-box">
		<!-- 查询表单 card -->
		<v-search @get-data="getData" @reset-change="reset" v-show="isShowSearch" :form-items="formItems" :form-data="formData" />
		<v-table ref="proTable" title="药采结算申请" :columns="columns" :request-api="getTableList"></v-table>
	</div>
</template>
...
const formItems = computed(() => {
	return [
		...
		},
		{
			...
			list: [
				{ value: '', label: '全部' },
				{ value: '0', label: '未入账', search: { disabled: true } },
				{ value: '1', label: '已入账' },
				{ value: '-12', label: '3标记不处理' },
				{ value: '12', label: '3已入账' },
				{ value: '-10', label: '5标记不处理' }
			],
			search: { span: 2, disabled: false }
		},
		{
			htmlType: 'moneyRange',
			label: '借方金额',
