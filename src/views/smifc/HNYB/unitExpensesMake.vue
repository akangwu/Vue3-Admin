    ...
    <el-dialog v-model="visibleAdd" title="新增基金申请单" class="dialog-form w-1200" v-if="visibleAdd">
        ...
        <Grid ref="gridRef" :gap="[20, 0]" :cols="3">
          ...
          <GridItem>
            <el-form-item label="申请后账户余额（万元）：" prop="afterRequestTotalAmount">
              <span>{{
                proxy.funcs.thousandPoint(formDataAdd.afterRequestTotalAmount ? formDataAdd.afterRequestTotalAmount : 0)
              }}</span>
            </el-form-item>
          </GridItem>
          <GridItem>
      ...
      <v-table ref="proTable1" ifIndex if-select :column="columnAdd" :data="tableDataAdd" isDialogTable height="650px">
        ...
        <template #businessItemCode="scope">
          <el-select
            style="width: 100%"
            v-model="scope.row.businessItemCode"
            placeholder="请选择基金支付类型"
            @change="handleBusinessItemChange(scope.row)"
          >
        ...
        <template #accountName="scope">
          <el-select
            ...
            placeholder="请选择户名"
            @change="handleAccountNameChange(scope.row)"
          >
            <el-option
              v-for="val in accountList"
              :label="`[${val.subAccountNo}]${val.subAccountName}`"
              :value="val.subAccountNo"
              :key="val.subAccountNo"
            ></el-option>
          </el-select>
        </template>
        <!-- 支出户余额 -->
        ...
        <template #coefficient="scope">
          <el-input
            ...
            placeholder="请填写系数"
            @blur="coefficientChangeHandle(scope.row)"
          ></el-input>
        </template>
  ...
  if (Object.keys(rowDataAccount).length) {
    ...
    if (row.monthPayAmount != -1) {
      ...
      row.afterRequestBalAmount =
        proxy.funcs.moneyTransferNum(row.monthAmount) +
        (proxy.funcs.moneyTransferNum(row.monthPayAmount) <= 0 ? 0 : proxy.funcs.moneyTransferNum(row.monthPayAmount));
